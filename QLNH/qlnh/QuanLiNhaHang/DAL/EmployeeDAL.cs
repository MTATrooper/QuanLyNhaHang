...
namespace QuanLiNhaHang.DAL
{
    class EmployeeDAL
    {
        ...
        {
            string query = string.Format("insert Employee(Name, Address, Age, PhoneNumber) values(N'{0}',N'{1}',{2},'{3}')",emp.Name, emp.Address, emp.Age, emp.PhoneNumber);
        ...
        {
            string query = string.Format(" update Employee set Name=N'{1}', Address= N'{2}', Age= {3}, PhoneNumber='{4}' where Id={0}", emp.Id, emp.Name, emp.Address, emp.Age, emp.PhoneNumber);
            dbConnect.ExecuteNonQuery(query);
        }
        // xoa nhan vien
